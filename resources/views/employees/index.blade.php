                    ...
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <div class="flex space-x-2">
                            ...
                            @if (Auth::user()->role === 'admin' || Auth::user()->role === 'company')
                                <a href="{{ route('employees.create', ['company_id' => Auth::user()->role === 'company' ? Auth::user()->company_id : request('company_id')]) }}" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-md">
                                    Add Employee
                                </a>
                            @endif
                        </div>
                                ...
                                @foreach ($employees as $employee)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900 dark:text-gray-200">{{ $employee->name }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900 dark:text-gray-200">{{ $employee->company->name }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('employees.edit', $employee->id) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-600">Edit</a>
                                            <form action="{{ route('employees.destroy', $employee->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure?');">
                                                @csrf
                                                @method('DELETE')
